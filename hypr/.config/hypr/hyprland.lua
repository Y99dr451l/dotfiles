...
hl.on("config.reloaded", function()
	restart("waybar", "hyprpaper", "swaync")
...
hl.window_rule({ name = "yt-opacity", match = { class = "firefox", title = ".*(YouTube|Watch2Gether|Picture-in-Picture|vesktop).*" }, opacity = "1. override", no_dim = true })
...
hl.bind(mainMod .. " + SHIFT + Y", function() local u = ({"low", "normal", "critical"})[math.random(3)]; local a = math.random(3); hl.exec_cmd("notify-send test -a App" .. a .. " -u " .. u) end, { repeating = true })
...
---- groups
local function movegroup(dir)
	if hl.get_active_window().group ~= nil then
		hl.dispatch(hl.dsp.window.move({ out_of_group = dir }))
	else hl.dispatch(hl.dsp.window.move({ into_group = dir }))
end end
hl.bind(mainMod .. " + ALT + left",  function() movegroup("left") end)
hl.bind(mainMod .. " + ALT + right", function() movegroup("right") end)
hl.bind(mainMod .. " + ALT + up",    function() movegroup("up") end)
hl.bind(mainMod .. " + ALT + down",  function() movegroup("down") end)
...
hl.bind(mainMod .. " + KP_ADD",      function() zoom(zoom_factor) end, { repeating = true })
hl.bind(mainMod .. " + KP_SUBTRACT", function() zoom(1. / zoom_factor) end, { repeating = true })
hl.bind(mainMod .. " + mouse_up",    function() zoom(zoom_factor) end )
hl.bind(mainMod .. " + mouse_down",  function() zoom(1. / zoom_factor) end )
