- clone this repo
- install packages dependencies given below
- delete all relevant files in `~` and `~/.config/` that are present in this repo
- run `stow --dotfiles -t ~ <configs>` in this repo to symlink configs
	...
	- `stow.sh` symlinks all configs to `~`
...
dependencies of `qt`: `breeze`, `qt6ct-kde`

dependencies of `gtk`: `breeze-gtk`, `qt6ct-kde` (idky)